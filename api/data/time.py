...
class Time:
    ...
    @staticmethod
    def days_into_month(timestamp: Optional[int]=None) -> int:
        """
        Returns an integer representing the number of days into the current month
        we are. If a timestamp is provided, returns the same value from that
        reverence point instead of now.
        """
        if timestamp is None:
            date = datetime.datetime.utcnow().date().timetuple()
        else:
